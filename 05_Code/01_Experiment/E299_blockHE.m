...
for t= next_trial:next_trial+nTrials-1
    ...
    tIntensity  = log10(.2);%exp.intensitites(side,result.trial_int(t));
    ...
    if response==0
      ...
      % response is anatomical
      % blockType 1 -ezes open 2- eyes closed
      % limbside (anatomical) 1 - left 2 - right
      % crossed 0 - uncrossed 1 - crossed
      if (result.trial_limbside(t) == 1 && result.trial_response(t) == 1) || ...
          (result.trial_limbside(t) == 2 && result.trial_response(t) == 2) 
           result.trial_correct(t)      = 1;
           display(sprintf('Correct\n'))
        elseif (result.trial_limbside(t) == 1 && result.trial_response(t) == 2) || ...
          (result.trial_limbside(t) == 2 && result.trial_response(t) == 1) 
