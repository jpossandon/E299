...
for (c in c(1,0)){
  ...
  #geom_violin(data=auxdat1,aes(x=Condition, y=meanRT),draw_quantiles = c(0.25, 0.5, 0.75),trim = FALSE,adjust = .9,scale='width')+
    geom_point(data=auxdat1,aes(x=ConditionJit, y=meanRT),fill=cbbPalette[xs],size=2,shape=21, stroke=.1, alpha = .5) +
  ...
  print(p1)
  saveGraph(file=paste(getwd(),"/figures/",pathfig[t],"/allSubjMeans",tsks[t],prf[c+1],sep=""), type="pdf")
  ...
  saveGraph(file=paste(getwd(),"/figures/",pathfig[t],"/allSubjsbytrial",tsks[t],prf[c+1],sep=""), type="pdf")
  }
  }
}