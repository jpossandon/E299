...
# Performance all conditions, plots for correct and incorrect for both tasks
  ...
  #  datFrameaux = subset(datFrameaux,trial_correct==c)
    ...
    dat1        = ddply(datFrameaux, .(subjIndx,cond,RespM,LegC,HandC,cLxL,cSxB), summarize,  errors=1-sum(trial_correct==1,na.rm=T)/sum(!is.nan(trial_correct)))}
    datperInt   = ddply(datFrameaux, .(subjIndx,cond,trial_int), summarize,  errors=1-sum(trial_correct==1,na.rm=T)/sum(!is.nan(datFrameaux$trial_correct)))
  openGraph(width = 3.5, height = 3) 
   # fig <- image_graph(width = 400, height = 400, res = 96)
    ...
    #             fill = "cyan", alpha = 0.3)
    xs <-1
    dat2 = ddply(dat1, .(cond), summarize,  meanError=mean(errors, na.rm=T),  median=median(errors, na.rm=T))
    ...
    for (ss in levels(dat1$cond)){               # loop through each condition
      ...
      thispos    <-  unique(auxdat1$Condition) 
        ...
        auxmcmc <- 1-inv.logit(this_mcmcChain[,sprintf("m[%d,%d,%d]",unique(auxdat1$RespM),unique(auxdat1$LegC),unique(auxdat1$HandC))])#}
    ...
    #  postMd$y    = (postMd$y/max(postMd$y)*rzfact)+thispos
      ...
      p1 <- p1 +
         geom_point(data=auxdat1,aes(y=ConditionJit, x=errors),fill='gray',size=1.5,shape=21, stroke=.1, alpha = .2) +         # plot each subjecy
      ...
      #  geom_point(data=auxmcmc,aes(x=x, y=mean),color='black',size=2,shape="-", stroke=.2) +  
         geom_text(data=auxframe,aes(y = x, x=ymax+.025,label = sprintf('%d',round(mean*100))),vjust=0.5,  # text RT
                  size=3)+
        ...
        geom_point(data=auxdat1,aes(y=Condition,x=mean(errors)),size=.75,shape=23,color="black",fill="yellow", stroke=.1) + # plot mean
        geom_point(data=auxdat1,aes(y=Condition,x=mean(errors)),size=.05,shape=20,color="black", stroke=.1)+  # plot mean
        coord_flip()
   ...
   if (inttype==1){
    ...
    # line between subject data in the same response mode
      ...
      p1$layers <- c(geom_path(data=auxdat3,aes(y=numcond+rep(jitterVal,4), x=errors, group=subjIndx),color='gray88', alpha = 1 ,size=.2), p1$layers)
  ...
  #  }
     ...
     p1 <- p1 + 
       ...
       scale_x_continuous(limits=c(0,.65),breaks=c(0,.2,.4,.6,.8,1),labels=c(0,.2,.4,.6,.8,1),expand = c(0, 0)) +
    ...
    print(p1)
    if (doublePlot){
      saveGraph(file=paste(path,"/07_Analyses/",pathfig[t],"/allSubjPerfInt",tsks[t],"confcond",sep=""), type="pdf")
      
    }else{
    saveGraph(file=paste(path,"/07_Analyses/",pathfig[t],"/allSubPerfInt",tsks[t],model,sep=""), type="pdf")
}
   }
