...
for (t in c(1:2)){
  ...
  dat1    = ddply(datFrameaux, .(subjIndx,cond), summarize, 
  ...
  for (ss in c(1,2,3,4)){
    ...
    p1 <- p1 + 
      geom_text(data=NULL,aes(x = 6+gap), y = .6-ss/25,label = crossLabels[ss],hjust=0,
  ...
  print(p1)
  saveGraph(file=paste(getwd(),"/figures/",pathfig[t],"/allSubjPerf",tsks[t],sep=""), type="pdf")
  }