...
model {
for ( i in 1:Ndata ) {
y[i] ~ dt( mu[i] ,1/(ySigma[cLxL[i],LegC[i],HandC[i],Int[i]])^2, nu)
#y[i] ~ dnorm( mu[i] ,1/(ySigma[cLxL[i],LegC[i],HandC[i],Int[i]])^2)
mu[i] <- a0 + a1[cLxL[i]] + a2[LegC[i]] + a3[HandC[i]] + a4[Int[i]] + aS[S[i]] +       # this is the full model with all interaction, included the interaction with the subject factor, this is only possible for when there is multiple data points per subject per cell 
a1a2[cLxL[i],LegC[i]] + a1a4[cLxL[i],Int[i]] + a1a3[cLxL[i],HandC[i]] + a2a3[LegC[i],HandC[i]] + a2a4[LegC[i],Int[i]] + a3a4[HandC[i],Int[i]] +
a1a2a3[cLxL[i],LegC[i],HandC[i]] +a1a2a4[cLxL[i],LegC[i],Int[i]] +a2a3a4[LegC[i],HandC[i],Int[i]] + a1a3a4[cLxL[i],HandC[i],Int[i]] +
a1a2a3a4[cLxL[i],LegC[i],HandC[i],Int[i]] 
}
#
nu <- nuMinusOne+1
