...
model {
for ( i in 1:Ndata ) {
y[i] ~ dt( mu[i] ,1/(ySigma[RespM[i],LegC[i],HandC[i],Int[i]])^2, nu)
#y[i] ~ dnorm( mu[i] ,1/(ySigma[RespM[i],LegC[i],HandC[i],Int[i]])^2)
mu[i] <- a0 + a1[RespM[i]] + a2[LegC[i]] + a3[HandC[i]] + a4[Int[i]] + aS[S[i]] +       # this is the full model with all interaction, included the interaction with the subject factor, this is only possible for when there is multiple data points per subject per cell 
...
loglik[i] <- logdensity.t(y[i], mu[i],1/(ySigma[RespM[i],LegC[i],HandC[i],Int[i]])^2,nu)
