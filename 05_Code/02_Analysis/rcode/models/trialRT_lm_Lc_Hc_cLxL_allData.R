...
y[i] ~ dnorm( mu[i] ,1/(ySigma[LegC[i],HandC[i],cLxL[i]])^2)
mu[i] <- a0 + a1[LegC[i]] + a2[HandC[i]]  + a3[cLxL[i]]  +  aS[S[i]] +        # this is the full model with all interaction, included the interaction with the subject factor, this is only possible for when there is multiple data points per subject per cell 
a1a2[LegC[i],HandC[i]] + a1a3[LegC[i],cLxL[i]] + a2a3[HandC[i],cLxL[i]] + 
a1a2a3[LegC[i],HandC[i],cLxL[i]]
}
#
#nu <- nuMinusOne+1
