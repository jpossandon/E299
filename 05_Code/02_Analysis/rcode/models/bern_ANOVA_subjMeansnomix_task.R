...
y[i] ~ dbern(p[i])
logit( p[i] ) <- Xbeta[i] 
Xbeta[i] <- a0 + a1[RespM[i]] + a2[LegC[i]] + a3[HandC[i]]  + a4[task[i]] + aS[S[i]] +        # this is the full model with all interaction, included the interaction with the subject factor, this is only possible for when there is multiple data points per subject per cell 
