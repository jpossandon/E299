...
mu[i] <- a0 + a1[cLxL[i]] + a2[cLcSxBcH[i]] +# a3[cLcSxL[i]] + a4[cLxBcH[i]] +  aS[S[i]] +        # this is the full model with all interaction, included the interaction with the subject factor, this is only possible for when there is multiple data points per subject per cell 
a1aS[cLxL[i],S[i]] + a2aS[cLcSxBcH[i],S[i]] #+ a3aS[cLcSxL[i],S[i]] + a4aS[cLxBcH[i],S[i]]
}
...
for ( j2 in 1:8 ) { a2[j2] ~ dnorm( 0.0 , 1/a2SD^2 ) }
a2SD ~ dgamma(aGammaShRa[1],aGammaShRa[2]) 
for ( j3 in 1:4 ) { a3[j3] ~ dnorm( 0.0 , 1/a3SD^2 ) }
a3SD ~ dgamma(aGammaShRa[1],aGammaShRa[2]) 
for ( j4 in 1:4 ) { a4[j4] ~ dnorm( 0.0 , 1/a4SD^2 ) }
...
for ( j1 in 1:2) { for ( j2 in 1:8 ) { for ( j3 in 1:4 ) {  for ( j4 in 1:4 ) { for ( js in 1:NSubj ) {
mm[j1,j2,j3,j4,js] <- a0 + a1[j1] + a2[j2] + a3[j3] + a4[j4] + aS[js] + # cell means 
a1aS[j1,js] + a2aS[j2,js] + a3aS[j3,js] + a4aS[j4,js] 
} } } } }

b0 <- mean( mm[1:2,1:8,1:4,1:4,1:NSubj] )
for ( j1 in 1:2 ) { b1[j1] <- mean( mm[j1,1:8,1:4,1:4,1:NSubj] ) - b0 }
for ( j2 in 1:8 ) { b2[j2] <- mean( mm[1:2,j2,1:4,1:4,1:NSubj] ) - b0 }
for ( j3 in 1:4 ) { b3[j3] <- mean( mm[1:2,1:8,j3,1:4,1:NSubj] ) - b0 }
for ( j4 in 1:4 ) { b4[j4] <- mean( mm[1:2,1:8,1:4,j4,1:NSubj] ) - b0 }
for ( js in 1:NSubj ) { bS[js] <- mean( mm[1:2,1:8,1:4,1:4,js] ) - b0 }

for ( j1 in 1:2) { for ( j2 in 1:8 ) { for ( j3 in 1:4 ) {for ( j4 in 1:4 ) {
