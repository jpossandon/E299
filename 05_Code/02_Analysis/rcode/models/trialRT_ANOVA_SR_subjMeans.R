...
m[j1,j2,j3,j4,j5,j6] <- a0 + a1[j1] + a2[j2] + a3[j3] + a4[j4]+ a5[j5]+ a6[j6]  # cell means 
} } } } } }
b0 <- mean( m[1:2,1:2,1:2,1:2,1:2,1:2] )
for ( j1 in 1:2 ) { b1[j1] <- mean( m[j1,1:2,1:2,1:2,1:2,1:2] ) - b0 }
for ( j2 in 1:2 ) { b2[j2] <- mean( m[1:2,j2,1:2,1:2,1:2,1:2] ) - b0 }
for ( j3 in 1:2 ) { b3[j3] <- mean( m[1:2,1:2,j3,1:2,1:2,1:2] ) - b0 }
for ( j4 in 1:2 ) { b4[j4] <- mean( m[1:2,1:2,1:2,j4,1:2,1:2] ) - b0 }
for ( j5 in 1:2 ) { b5[j5] <- mean( m[1:2,1:2,1:2,1:2,j5,1:2] ) - b0 }
for ( j6 in 1:2 ) { b6[j6] <- mean( m[1:2,1:2,1:2,1:2,1:2,j6] ) - b0 }
