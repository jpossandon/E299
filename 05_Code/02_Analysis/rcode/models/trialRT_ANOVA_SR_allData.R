...
for ( j1 in 1:2) { for ( j2 in 1:2 ) { for ( j3 in 1:2 ) {  for ( j4 in 1:2 ) { for ( j5 in 1:2 ) { for ( j6 in 1:2 ) { for (js in 1:NSubj){
mm[j1,j2,j3,j4,j5,j6,js] <- a0 + a1[j1] + a2[j2] + a3[j3] + a4[j4]+ a5[j5]+ a6[j6] + aS[js] + # cell means 
a1aS[j1,js] + a2aS[j2,js] + a3aS[j3,js] + a4aS[j4,js] + a5aS[j5,js] + a6aS[j6,js]   
} } } } } } }

b0 <- mean( mm[1:2,1:2,1:2,1:2,1:2,1:2,1:NSubj] )
for ( j1 in 1:2 ) { b1[j1] <- mean( mm[j1,1:2,1:2,1:2,1:2,1:2,1:NSubj] ) - b0 }
for ( j2 in 1:2 ) { b2[j2] <- mean( mm[1:2,j2,1:2,1:2,1:2,1:2,1:NSubj] ) - b0 }
for ( j3 in 1:2 ) { b3[j3] <- mean( mm[1:2,1:2,j3,1:2,1:2,1:2,1:NSubj] ) - b0 }
for ( j4 in 1:2 ) { b4[j4] <- mean( mm[1:2,1:2,1:2,j4,1:2,1:2,1:NSubj] ) - b0 }
for ( j5 in 1:2 ) { b5[j5] <- mean( mm[1:2,1:2,1:2,1:2,j5,1:2,1:NSubj] ) - b0 }
for ( j6 in 1:2 ) { b6[j6] <- mean( mm[1:2,1:2,1:2,1:2,1:2,j6,1:NSubj] ) - b0 }
for ( js in 1:NSubj ) { bS[js] <- mean( mm[1:2,1:2,1:2,1:2,1:2,1:2,js] ) - b0 }

for ( j1 in 1:2) { for ( j2 in 1:2 ) { for ( j3 in 1:2 ) {for ( j4 in 1:2 ) { for ( j5 in 1:2 ) { for ( j6 in 1:2 ) { 
m[j1,j2,j3,j4,j5,j6]<- mean(mm[j1,j2,j3,j4,j5,j6,1:NSubj])
}}}}}}
