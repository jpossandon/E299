...
model {
for ( i in 1:Ndata ) {
y[i] ~ dt( mu[i] ,1/(ySigma[cLxL[i],LegC[i],HandC[i]])^2, nu)
#y[i] ~ dnorm( mu[i] ,1/(ySigma[cLxL[i],LegC[i],HandC[i]])^2)
mu[i] <- a0 + a1[cLxL[i]] + a2[LegC[i]] + a3[HandC[i]]  + aS[S[i]] +       # this is the full model with all interaction, included the interaction with the subject factor, this is only possible for when there is multiple data points per subject per cell 
a1a2[cLxL[i],LegC[i]] + a1a3[cLxL[i],HandC[i]] + a2a3[LegC[i],HandC[i]]  +
a1a2a3[cLxL[i],LegC[i],HandC[i]] 

regression_residual[i] <- y[i] - mu[i]
loglik[i] <- logdensity.t(y[i], mu[i],1/(ySigma[cLxL[i],LegC[i],HandC[i]])^2,nu)
}
#
nu <- nuMinusOne+1
...
for ( j1 in 1:2) { for ( j2 in 1:2 ) { for ( j3 in 1:2 ) {
b1b2b3[j1,j2,j3] <-  mean(mm[j1,j2,j3,1:NSubj]) - ( b0 + b1[j1] + b2[j2] + b3[j3] + b1b2[j1,j2] + b1b3[j1,j3] + b2b3[j2,j3] )
}}}
