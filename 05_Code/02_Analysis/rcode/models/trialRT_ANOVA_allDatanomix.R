...
mu[i] <- a0 + a1[RespM[i]] + a2[LegC[i]] + a3[HandC[i]] +        # this is the full model with all interaction, included the interaction with the subject factor, this is only possible for when there is multiple data points per subject per cell 
...
mm[j1,j2,j3] <- a0 + a1[j1] + a2[j2] + a3[j3] + 
...
b0 <- mean( mm[1:2,1:2,1:2] )
for ( j1 in 1:2 ) { b1[j1] <- mean( mm[j1,1:2,1:2] ) - b0 }
for ( j2 in 1:2 ) { b2[j2] <- mean( mm[1:2,j2,1:2] ) - b0 }
for ( j3 in 1:2 ) { b3[j3] <- mean( mm[1:2,1:2,j3] ) - b0 }
#for ( js in 1:NSubj ) { bS[js] <- mean( mm[1:2,1:2,1:2,js] ) - b0 }
m <- mm


for ( j1 in 1:2 ) { for ( j2 in 1:2 ) {
b1b2[j1,j2] <- mean(mm[j1,j2,1:2]) - ( b0 + b1[j1] + b2[j2] )
}}

for ( j1 in 1:2 ) { for ( j3 in 1:2) {
b1b3[j1,j3] <- mean(mm[j1,1:2,j3]) - ( b0 + b1[j1] + b3[j3] )
}}

for ( j2 in 1:2 ) { for ( j3 in 1:2 ) {
b2b3[j2,j3] <- mean(mm[1:2,j2,j3]) - ( b0 + b2[j2] + b3[j3] )
}}
